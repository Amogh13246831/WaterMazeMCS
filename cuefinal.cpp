...
class PhysData          //all physical parameters
{
 ...
 static const int numsubjects;// number of subjects in the trial
...
const int PhysData::maxcues = 10; // an upper bound to the number of cues

const int PhysData::numsubjects = 10;
...
class Arena: public PhysData // the water maze, and related operations
{
 ...
 void get_comwt();            // compute Wcom
 void get_platwt();           // compute Wplat 
...
class Sim: public Rat, public Arena // a rat, the maze, and their interactions 
{
 ...
 void initialise(int id);   // set up a simulation
 ...
 */
 ...
 cout<<"\nDATA AFTER WHILE: "<<x<<" "<<y<<endl;

 cout<<"Random angle: "<<rad_to_deg(theta)<<"\t"<<(int)x<<" "<<(int)y<<endl;
 if(numcues)
 {
  cout<<"Visual cue angles: ";
  for(i=0; i<numcues; i++)
  {
   cout<<rad_to_deg(viscues[i].randvect[curx][cury])<<"\t";
   cout<<cuestep[i][0]<<" "<<cuestep[i][1]<<"\t\t";
  }
  cout<<endl;
  if(best != -1)
  {
   cout<<"Best value from cue "<<best<<", value: "<<score<<endl;
 ...
 */
 cout<<"angle old cell "<<curx<<" "<<cury<<":\t"<<arena[curx][cury].dirvect<<endl;
 ...
 cout<<"angle new cell "<<curx<<" "<<cury<<":\t"<<arena[curx][cury].dirvect<<endl;
...
void Sim::run_simulation()
{
 ...
 get_comwt();             // compute weight to store
 get_platwt();      // compute inverse distance of CoM from platform
 print_arena();           // print experiment parameters and trial results
 update_memory();         // update trial results to stored arena
 print_stored();
 update_cues();
 put_cues();
 cout<<"Total number of successes: "<<successes<<endl;
}
...
void Sim::read_from_file(int number)
{
 ...
 if(!in)
 {
  cout<<"Could not open file "<<fstr<<endl;
  isfirsttrial = true;
  return;
 }
...
int main()
{
 sleep(1);                     // ensure new RNG seed
 srand(time(NULL));            // get RNG seed

 int n = PhysData::numsubjects;
 Sim subjects[n];              // trial subjects

 for(int trial=0; trial<1; trial++)
  for(int i=0; i<n; i++)
  {
   subjects[i].read_from_file(i);
   subjects[i].initialise(i);
   subjects[i].run_simulation();
   subjects[i].store_to_file();
  }
 return 0;
}
