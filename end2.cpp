...
class PhysData          //all physical parameters
{
 ...
 static const int maxsubjects;// number of subjects in the trial
...
const int PhysData::maxsubjects = 10;
...
void CueData::platform_relative(int px, int py)
{ 
...
int main()
{
 ...
 int msub = PhysData::maxsubjects;
 int numSub, numTrials;
 Sim subjects[msub];              // trial subjects


 system("clear");
 cout<<"WATER MAZE SIMULATOR\n\n\n";

 cout<<"Enter number of trial subjects: ";
 cin>>numSub;
 if(numSub > msub)
 {
  cout<<"Too many subjects: number set to "<<msub<<endl;
  numSub = msub; 
 }

 cout<<"Enter number of trials to be performed: ";
 cin>>numTrials;
 
 for(int trial=0; trial<numTrials; trial++)
  for(int sub=0; sub<numSub; sub++)
  {
   subjects[sub].read_from_file(sub);
   subjects[sub].initialise(sub);
   subjects[sub].run_simulation();
   subjects[sub].store_to_file();
  }
 
 for(int sub=0; sub<numSub; sub++)
  subjects[sub].print_info();
